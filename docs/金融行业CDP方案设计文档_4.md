...
h1. 数据采集平台（Ingestion） {anchor:数据采集平台Ingestion}
...
** 弱标识关联（同会话/时空/行为）
** 人工校对流程
* *OneID图谱与关系存储*：ES 文档型 + 边索引，或独立图数据库（可选）。
...
* 成本策略：热数据3 hippocampus 数据归档OSS、TTL自动清理
...
*其他角色*：运营、客服、第三方接入（APIInstance用法：{{org_id}}, {{one_id}}, {{event_id}}, {{occurred_at}}, {{source}}, {{ip}}, {{ua}}
* *主数据必填与校验规则*（Schema Registry）
...
*事件清单示例*：
* CRM事件：{{call.outbound.started}}, {{call.inbound.received}}, {{lead.created}}, {{opportunity.won}} 等渐进式

----

h1. 附录：核心数据模型（简化） {anchor:附录核心数据模型简化}
