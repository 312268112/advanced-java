...
- [概述](#h-概述)
- [多租户与隔离模型](#h-多租户与隔离模型)
- [数据采集平台](#h-数据采集平台ingestion)
- [OneID 与实体解析](#h-oneid-与实体解析)
- [数据中台架构](#h-数据中台架构)
- [数据分析平台](#h-数据分析平台)
- [数据应用平台与触达](#h-数据应用平台与触达)
- [组织架构与上级决策人定位](#h-组织架构与上级决策人定位)
- [安全与合规](#h-安全与合规)
- [数据治理与质量](#h-数据治理与质量)
- [运维与SLA](#h-运维与sla)
- [技术栈与落地建议](#h-技术栈与落地建议)
- [里程碑与路线图](#h-里程碑与路线图)
- [标准与规范](#h-标准与规范)
- [接入清单与实施计划](#h-接入清单与实施计划)
- [权限矩阵](#h-权限矩阵)
- [数据保留与删除](#h-数据保留与删除)
- [附录：核心数据模型](#h-附录核心数据模型简化)
- [可配置项与下一步](#h-可配置项与下一步)

## 概述

- **目标**：建设面向金融行业的 ToB CDP，覆盖数据采集 → OneID → 数据中台 → 数据分析 → 数据应用的全链路，满足多租户隔离、企业级合规与实时-离线混合场景。
- **特点**：
...
- **跳跃源现状**：CRM（类呼叫中心）、企微；后续陆续接入第三方数据（广告、线索、工商、行为、支付、风控等）。
...
- **合并策略**：
  ...
  - 弱标识关联（同会话/时空/行为）
  - 人工校对流程
lineID图谱与关系存储：ES 文档型 + 边索引，或独立图数据库（可选）。
...
- **既有引擎利用**：
  ...
  - ES：行为检索、人群反查、近实时画像。
  - 可选 OLAP：ClickHouse/Dhews/StarRocks，用于重型明细分析与分群。
- **实时与离线**：
...
```mermaid
flowchart TB
  A[企微组织/成员关系] --> corporations/上下级规则]
...
- **元自据/血缘/版本**（Schema Registry、指标字典、口径版本化、可视化血缘）
- **数据质量**：
...
- 企挨组织：wecom_org(dept_id, parent_dept_id, level, manager_id, ...)
